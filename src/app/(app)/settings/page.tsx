...
export default function SettingsPage() {
  ...
  async function regenerate() {
    ...
    if (!confirm("Regenerating will rebuild your plan and reset progress. This calls the AI (costs apply). Continue?"))
      return;
    setError(null);
    setBusy(true);
  ...
  function deletePlan() {
    if (!confirm("Delete your plan permanently?")) return;
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-6 lg:px-8">
      ...
      <section className="card p-6">
        ...
        <p className="mb-4 text-xs text-ink-soft">Editing these regenerates your plan.</p>

        <label className="block text-sm font-medium text-ink">Goal</label>
        <textarea
          value={goal}
        ...
        <label className="mt-4 block text-sm font-medium text-ink">Level</label>
        ...
        <label className="mt-4 block text-sm font-medium text-ink">Resource preference</label>
      ...
      <section className="mt-6 card border-red-200 p-6">
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          ...
          >
            <Download size={15} /> Export plan (JSON)
          </button>
          <button
            onClick={deletePlan}
