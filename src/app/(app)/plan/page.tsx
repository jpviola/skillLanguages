...
export default function DashboardPage() {
  ...
  if (!state.plan || !state.userProfile) {
    return <div className="p-8 text-sm text-ink-soft">No plan yet. Redirecting…</div>;
  }
  ...
  const estFinish =
    weeksRemaining > 0
      ? new Date(Date.now() + weeksRemaining * 7 * 86_400_000).toLocaleDateString(undefined, {
          month: "short",
          day: "numeric",
          year: "numeric",
        })
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-ink-soft">
          ...
          {estFinish && (
            <>
              {" · "}
              <span className="font-medium text-ink">Est. finish {estFinish}</span>
            </>
          )}
        </p>
      </div>
      ...
      <section className="mb-6 overflow-hidden rounded-card bg-linear-to-r from-indigo-600 to-indigo-500 p-5 text-white shadow-elevated">
        {focusTopic ? (
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-medium uppercase tracking-wide text-indigo-200">
                Focus now · Week {currentWeek.week_number} ({curDone}/{currentWeek.topics.length} done)
              </p>
              <p className="mt-1 truncate text-lg font-bold">{focusTopic.name}</p>
              <p className="text-sm text-indigo-100">
                {focusTopic.type} · {focusTopic.estimated_minutes} min — {currentWeek.title}
              </p>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold">All planned weeks complete 🎉</p>
              <p className="text-sm text-indigo-100">
                Submit feedback below to extend or refine your path.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <section className="card order-1 p-5 lg:col-span-2 lg:col-start-1 lg:row-start-1">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge icon={<Coins size={13} />} label={userProfile.resource_preference} />
          </div>
        ...
        <section className="order-3 lg:col-span-2 lg:col-start-1 lg:row-start-2">
          <h2 className="mb-3 text-lg font-bold text-ink">Your Personalized Learning Path</h2>
        ...
        <div className="order-2 space-y-6 lg:col-start-3 lg:row-span-2 lg:row-start-1">
          ...
          <section className="card p-5">
            <h2 className="mb-3 text-sm font-semibold text-ink">Your Progress</h2>
            <div className="flex items-center gap-5">
              <ProgressRing percent={overall} />
              <div className="space-y-2 text-sm">
                <Stat color="bg-emerald-500" label="Completed" value={counts.completed} />
                <Stat color="bg-amber-500" label="In Progress" value={counts.inProgress} />
                <Stat color="bg-slate-300" label="Not Started" value={counts.notStarted} />
              </div>
            </div>
          </section>
          ...
          {nextWeek && (
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-ink">
                Next Up — Week {nextWeek.week_number} Preview
              </h2>
              ...
              >
                View Full Week <ArrowRight size={14} />
              </Link>
            </section>
          )}
          ...
          <section className="card flex items-center gap-4 bg-primary-light p-5">
            ...
            <p className="text-sm text-ink">
              <span className="font-semibold">Stay consistent!</span> 30 min daily beats 4 hours once
              a week.
            </p>
          </section>
